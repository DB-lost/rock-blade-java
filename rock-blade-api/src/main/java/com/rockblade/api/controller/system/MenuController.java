...
public class MenuController {
  ...
  public R<Boolean> nameExists(
    ...
    return R.ok(menuService.queryChain().where(MENU.NAME.eq(name)).and(MENU.ID.ne(id)).exists());
  }
  ...
  public R<Boolean> pathExists(
      ...
      @Parameter(description = "菜单id", required = false) @RequestParam(required = false) Long id) {
    return R.ok(menuService.queryChain().where(MENU.PATH.eq(path)).and(MENU.ID.ne(id)).exists());
  }
